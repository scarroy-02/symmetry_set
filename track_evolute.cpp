...
void saveAnimationData(const std::string& filename, 
    ...
    // Header
    file << "t,curve_x,curve_y,focal_x,focal_y\n";
    ...
    for (int i = 0; i <= resolution; i++) {
        ...
        // 1. Evaluate Curve Point
        Point p_curve = curve.evaluate(t);
        
        // 2. Evaluate Focal Point
        Point p_focal(NAN, NAN);
        Point fp = curve.computeFocalPoint(t, R_max);
        
        // Check if valid (not NAN)
        if (!std::isnan(fp.x) && !std::isnan(fp.y)) {
            p_focal = fp;
        }

        // Write synchronized row
        file << t << "," 
             << p_curve.x << "," << p_curve.y << ","
             << p_focal.x << "," << p_focal.y << "\n";
    }
    ...
    file.close();
    std::cout << "Animation data saved to " << filename << std::endl;
}

int main() {
    const double PI = 3.14159265358979323846;
    
    // Define the ellipse curve (same as your logic)
    double a = -0.55;
    double b = 0.6;
    ...
    // Lower resolution is usually better for animation smoothness vs file size
    // 1000 frames is a good balance for a 30-40 second animation
    saveAnimationData("monodromyfig4_evolute.csv", curve, 200000, 50000.0);
